...
* I realised over the weekend that I made a bit of a daft mistake with the shift left, as I am seeking backwards then reading forwards. Probably not an issue on SSDs but on a spinning platter that is going to be a recipe for slow performance. What I should have done is moved forward in chunks, shifting each left, but not written the last byte until I had completed the first byte of the next chunk. This would actually fit quite well with the parallelism below as well.
